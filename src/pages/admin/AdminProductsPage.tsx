...
export function AdminProductsPage() {
  ...
  return (
    <>
      <PageHeader
        title="Products"
        description="Manage DIY plans, physical shop products, and coaching programs."
      >
        <Link
      ...
      <Card className="p-0 overflow-hidden">
        ...
        {products.length > 0 && (
          <table className="w-full text-sm">
            ...
            <tbody>
              {products.map((p) => {
                const isPhysical = p.category === "physical" || p.format === "Physical";
                    ...
                    <td className="px-6 py-4">
                      {isPhysical ? (
                        p.stock != null && p.stock <= 0 ? (
                          <span className="inline-flex items-center rounded-full bg-coral-100 px-2.5 py-0.5 text-xs font-semibold text-coral-700">
                            Out of stock (0)
                          </span>
                        ) : p.stock != null && p.stock <= 5 ? (
                          <span className="inline-flex items-center rounded-full bg-coral-50 px-2.5 py-0.5 text-xs font-semibold text-coral-600">
                            Low stock ({p.stock})
                          </span>
                        ) : (
                          <span className="inline-flex items-center rounded-full bg-forest-50 px-2.5 py-0.5 text-xs font-medium text-forest-700">
                            {p.stock != null ? `Stock: ${p.stock}` : "In stock"}
                          </span>
                        )
                      ) : (
                        <span className="text-xs text-ink-muted">Digital</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      {p.isPublished ? (
